      ...
      for (const items in cartItems) {
        for (const item in cartItems[items]) {
          if (cartItems[items][item] > 0) {
            ...
            if (!itemInfo) {
              console.error(
                `Product with ID ${items} not found in products list.`
              );
              toast.error("Some items in your cart are no longer available.");
              return;
            }

            let clonedItemInfo = structuredClone(itemInfo);
            clonedItemInfo.size = item;
            clonedItemInfo.quantity = cartItems[items][item];
